...
class mambaFeedback(nn.Module):
    ...
    def forward(self, mem_x, mem_y, buffer_batch_size, classes_mean, task_id):
        ...
        if prob_sample_num != 0:
            ...
            nonZero_class = torch.arange(class_num).to(nonZeroRows.device)[nonZeroRows]
            # nonZero_class = torch.arange(class_num)[nonZeroRows]
            Zero_class = torch.arange(class_num).to(nonZeroRows.device)[ZeroRows]
    ...
    def make_mix_pair(self, sample_prob, prob_sample_num, nonZero_class, Zero_class, current_task_id):
        ...
        for task_id in range(start_i, end_i):
            ...
            if not any(indices):
                continue
            x_indices = torch.arange(self.buffer.x.shape[0]).to(indices.device)
            y_indices = torch.arange(self.buffer.y.shape[0]).to(indices.device)
