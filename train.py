...
from modules.APA import APALoss
from modules.MF import mambaFeedback
...
class TrainLearner(object):
    def __init__(self, model, buffer, optimizer, n_classes_num, class_per_task, input_size, args, fea_dim=128):
        ...
        self.OPELoss = APALoss(self.class_per_task, temperature=self.proto_t)
        ...
        self.MF = mambaFeedback(self.buffer, args.mixup_base_rate, args.mixup_p, args.mixup_lower, args.mixup_upper,
    ...
    def train_task0(self, task_id, train_loader):
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            with autocast():
                ...
                features, projections = self.model(images_pair, y,use_proj=True)
                ...
                y_pred = self.model(self.transform(x),y)
    ...
    def train_other_tasks(self, task_id, train_loader):
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            with autocast():
                ...
                if batch_idx != 0:
                    mem_x, mem_y, mem_y_mix = self.MF(ori_mem_x, ori_mem_y, buffer_batch_size, self.classes_mean, task_id)
                    rot_sim_labels = torch.cat([y + self.oop_base * i for i in range(self.oop)], dim=0)
                ...
                features, projections = self.model(all_images, y,use_proj=True)
                ...
                y_pred = self.model(self.transform(mem_x),y)
            ...
            if num_d % self.print_num == 0 or batch_idx == 1:
                print('==>>> it: {}, loss: ce {:.2f} , APA {:.4f} , {}%'
                    .format(batch_idx, ce, OPE_loss,  100 * (num_d / self.total_samples)))
    ...
    def cal_buffer_proto_loss(self, buffer_x, buffer_y, buffer_x_pair, task_id):
        buffer_fea, buffer_z = self.model(buffer_x_pair, buffer_y,use_proj=True)
